...
`scripts/sprint10e_v1_release_drill.ps1` 已完成 15/15 本地验收：Frontend 21/21，Backend 专项 Release Engineering 15/15、Dependency Lock 3/3、Schema Migration 10/10、Plugin Catalog 9/9、Plugin Runtime 12/12，Backend 全量 535/535（130.724 秒）。真实 `qwen3:8b` 旅程完成三阶段 candidate-only Planner 与显式接受；首次 Workflow 的 `review_parse_failed` 被保留审计，第二次通过完整质量门。Manuscript revision 1 经显式接受后完成 2/2 事实投影，10 文件导出两次确定性一致，manifest SHA-256 为 `f98d40b92f7b98868f3fa905543ba83ee92e83490042ac8544eae423c72f3e19`；Backend 重启持久性与用户隔离均通过。Hosted CI run `32040818117` 在 tag 提交 `f8e49e4` 全部成功；修复清单工作目录后，Hosted Release run `32043329937` 从 master workflow 对不可变 `v1.0.0` tag 完成手工重跑，非 draft、非 prerelease Release 已发布源码 ZIP、三镜像归档和 `SHA256SUMS`。最终 Go/No-Go 返回 `local_decision=go` 与 `distribution_decision=go`。
