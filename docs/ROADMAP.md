...
- FastAPI、Docker Compose、本地 Ollama/Qwen，以及 DeepSeek、OpenAI、Claude、DashScope 云 Provider 框架。
...
| 09C.3 | 小说导出 | 已完成 | 按用户/小说隔离导出当前规划与 accepted manuscript；版本 manifest、逐文件 SHA-256、候选排除和确定性下载可验证 |
| 09C | 迁移、备份与导出 | 已完成 | 多 SQLite/FAISS/Graph 一致快照、恢复演练、schema migration 与小说导出可验收 |
| 09D | CI 与发布工程 | 已完成 | 自动回归、镜像构建、确定性制品、checksum、升级/回滚和 tag 发布清单可验收 |
| 09 | Provider、Prompt、鉴权与发布工程 | 已完成 | 09A-09D 全部完成，聚合阶段关闭 |
| 10A | 插件契约与只读目录 | 已完成 | manifest、兼容策略、权限声明、精确启停和管理员 Catalog 可验收，且不执行第三方代码 |
| 10B | 受控插件运行时 | 待开发 | entry point 来源校验、能力级上下文、生命周期、故障隔离和回滚可验收 |
